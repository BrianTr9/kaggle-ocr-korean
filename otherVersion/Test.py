...
def _estimate_smallest_text_height_optimized(gray: np.ndarray) -> float | None:
    ...
    stats = stats[1:] # Bỏ qua background
...
def run_ocr(img_path, reader, chunk_height=1000, overlap=13): # overlap = 1 13 17
    ...
    if full_image is None:
        ...
        return []
    full_image = cv2.convertScaleAbs(full_image, alpha=0.7, beta=0) 
...
if __name__ == "__main__":
    ...
    # Bỏ comment dòng dưới để chạy toàn bộ thư mục
    # process_folder(INPUT_DIR, OUTPUT_DIR, reader)

    # Chạy thử nghiệm trên một ảnh
